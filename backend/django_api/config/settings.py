...
INSTALLED_APPS = [
    ...
    "trails",
    # Друзья (D-82, этап 2a): граф взаимных друзей.
    "friends",
    ...
    "staff",
    # Фотопайплайн товаров: загрузка исходников по артикулу → обработка ИИ → мастер+webp.
    "productmedia",
]
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        # Опрос канала входа, пока человек ждёт код: 20 запросов в минуту на человека,
        # за адресом оператора их бывает десяток. Брутфорсом не является — кода тут нет.
        "otp_poll": "300/min",
...
UNFOLD = {
    ...
    # Подгонка высоты таблицы под экран: полоса боковой прокрутки должна
    # оставаться над нижней панелью, иначе по ней нельзя попасть мышью.
    "SCRIPTS": [lambda request: versioned("admin/mata.js")],
        ...
        "navigation": [
            ...
            },
            {
                ...
                "items": [
                    ...
                    # Место — рядом с Конструктором: это наполнение витрины,
                    # а не наблюдение за системой.
            ...
            },
            {
                ...
                "items": [
                    ...
                    # Чего не хватает в карточках 1С: что просить заполнить.
                    {"title": "Заполненность 1С", "icon": "fact_check",
                     "link": reverse_lazy("onec_fill"),
                     "permission": _tab("onec_fill")},
                    # Что похоже на ошибку в данных: опечатки, странные размеры.
                    {"title": "Проверка данных", "icon": "rule",
                     "link": reverse_lazy("data_check"),
                     "permission": _tab("data_check")},
