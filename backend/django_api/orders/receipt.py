...
**Два чека, а не один (D-101).** Здесь собирается чек на ОПЛАТУ — это предоплата:
покупатель заплатил, но вещь ещё на складе. Второй чек, «полный расчёт» с зачётом
аванса, пробивает склад в 1С в момент сборки и отгрузки.

Маркировка. Одежда и обувь маркируются «Честным знаком»: код есть только у
конкретной вещи, и узнаётся он, когда кладовщик её отсканировал — то есть уже
после оплаты. Поэтому коды уходят во ВТОРОЙ чек, который пробивает 1С: там сканер
и там же коды списываются, ровно как в офлайн-магазине. Место для кода у нас
готово: если в позиции заказа есть `markCode`, он уйдёт в чек.
"""
import os
...
def _mode() -> str:
    """Признак способа расчёта в чеке оплаты.

    Интернет-магазин в момент оплаты получает ПРЕДОПЛАТУ: вещь ещё лежит на складе,
    её выберут и отгрузят позже. Поэтому `full_prepayment`, а не `full_payment` —
    иначе чек утверждает, что товар уже передан покупателю (D-101).

    Второй чек, «полный расчёт» с зачётом аванса и кодами маркировки, пробивает
    склад в 1С при сборке — там сканер и там же списываются коды «Честного знака».
    """
    return (os.environ.get("PAYMENT_MODE") or "full_prepayment").strip()
